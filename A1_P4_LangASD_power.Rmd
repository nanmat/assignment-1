...
#The power analysis reveals that there is no predictive power. The power curve returns the same results that even with more participants it is not possible to obtain a power over 80 %, not even a tendency that more participants would change the result. It seems that the visits and the diagnosis in itself has no predictive power, and only the interaction with visits and diagnosis has a power value that qualifies the model to predict the development. 
...
```{r}
#The previous exercise showed that the only relevant effect is the interaction and that would have a power value over 80 % with only 20 kids. Due to this it would be worth to run the study if interested in the interaction of diagnosis and visits but otherwise it would not be worth the effort. 
