...
```{r}
setwd("~/OneDrive/3 Semester/Experimental Methods 3/Assignment 1")
```
...
names(data1)[names(data1)=="Child.ID"] <- "ID"
names(data2)[names(data2)=="SUBJ"] <- "ID"
...
```{r}
library(stringr)
#Removing everything else than digits
...
#Selecting the variables from each dataset
data1_sub = select(data1, ID, Visit, Ethnicity, Diagnosis, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw)
data2_sub = select(data2, ID, Visit, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)
data3_sub = select(data3, ID, Visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI)

#Renaming two colums
names(data1_sub)[names(data1_sub)=="MullenRaw"] <- "nonverbalIQ"
names(data1_sub)[names(data1_sub)=="ExpressiveLangRaw"] <- "verbalIQ"
...
data4 = merge(data1_sub, data2_sub)
finaldata = merge(data4, data3_sub)
...
```{r}
#Extracting the data from visit 1
finaldata_sub = subset(finaldata, Visit == "1")
#Putting the values from visit 1 to the rest of the dataset
finaldata_sub = select(finaldata_sub, ID, ADOS, nonverbalIQ, verbalIQ)
finaldata = finaldata[,-7:-9]
finaldata_1 = merge(finaldata, finaldata_sub)
...
```{r}
finaldata_1$ID = as.numeric(as.factor(as.character(finaldata_1$ID)))

finaldata_1$Visit = as.numeric(as.character(finaldata_1$Visit))

#Making gender as a factor and changing the factors to M (male) and F (female)
finaldata_1$Gender = as.factor(finaldata_1$Gender)
finaldata_1 <- transform(finaldata_1,
...
```{r}
write.csv(finaldata_1, "finaldata.csv", sep =",")
```
...
finaldata=finaldata[,-1] #Omits the first column which we are not interested in
finaldata %>% group_by(ID) %>% summarise(MEAN = mean(CHI_MLU)) %>% filter(MEAN > 2.7)

#filter(finaldata_1, CHI_MLU > 2.7)

CHI_MLU_1.5 = filter(finaldata, Visit == 1, CHI_MLU < 1.5)
CHI_MLU_1.5$ID

missed = finaldata %>% group_by(ID) %>% tally()
CHI_MLU_missed = filter(missed, n < 6)
CHI_MLU_missed$ID
```
...
```{r}
visit_6 = subset(finaldata, finaldata$Visit == 6)
arrange(visit_6, c(visit_6$tokens_CHI))
#Kid 55 produced the most words on the 6th visit

visit_1 = subset(finaldata, Visit == "1")
arrange(visit_1, tokens_CHI)
#Kid 57 produced the most words on the 6th visit
...
```{r}
ASD_only = subset(finaldata, Diagnosis == "ASD")
ASD_only = select(ASD_only, MOT_MLU, CHI_MLU, tokens_MOT, tokens_CHI)

ASD_only2 = select(ASD_only, MOT_MLU, CHI_MLU, tokens_MOT, tokens_CHI, tokens_CHI)
#Nothing happens - The varible will only appear one time even though you have written it two times. 
...
```{r}
finaldata_2 = finaldata %>% group_by(Visit) %>% mutate(Mean_words_visit = sum(tokens_CHI)/length(tokens_CHI))

finaldata_3 = finaldata_2 %>% group_by(ID) %>% mutate(Mean_words_id = sum(tokens_CHI)/length(tokens_CHI))
finaldata_3

#We don't just use the average values because we end up not taking the previous visit into account. We would violate the assumption of independence. We cannot regard multiple recordings from the same subject as independent from each other. 
