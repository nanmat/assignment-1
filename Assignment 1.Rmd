...
```{r}
write.csv(finaldata_1, file = "finaldata.csv", sep = ",")
```
...
finaldata_1 %>% group_by(ID) %>%
summarise(MEAN = mean(CHI_MLU))
filter(MEAN > 2.7)

#filter(finaldata_1, CHI_MLU > 2.7)

filter(finaldata_1, Visit == 1, CHI_MLU < 1.5)

missed = finaldata_1 %>% group_by(ID) %>% tally()
filter(missed, n < 6)
...
```{r}
visit_6 = subset(finaldata_1, Visit == "6")
arrange(visit_6, tokens_CHI)
#Kid 55 produced the most words on the 6th visit

visit_1 = subset(finaldata_1, Visit == "1")
...
```{r}
ASD_only = subset(finaldata_1, Diagnosis == "ASD")
...
#The varible will only appear one time even though you have written it two times. 
...
```{r}
finaldata_2 = finaldata_1 %>% group_by(Visit) %>% mutate(Mean_words_visit = sum(tokens_CHI)/length(tokens_CHI))
